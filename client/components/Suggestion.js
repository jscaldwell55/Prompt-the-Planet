...
const Suggestion = ({ suggestion, onVote, onStatusChange }) => {
  ...
  const handleStatusChange = async (status) => {
    try {
      const response = await fetch(`/api/suggestions/${suggestion.id}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const updatedSuggestion = await response.json();
      onStatusChange(updatedSuggestion);
      setOpen(false);
    } catch (error) {
  ...
  return (
    ...
    >
      <CardContent>
        ...
        <SyntaxHighlighter
          language={suggestion.prompt.language}
          style={docco}
          customStyle={{
      ...
      <CardActions>
        ...
        {suggestion.status === 'OPEN' && suggestion.prompt.authorId === suggestion.authorId && (
          <Button
            variant="contained"
